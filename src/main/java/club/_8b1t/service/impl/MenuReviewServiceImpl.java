package club._8b1t.service.impl;

import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import club._8b1t.model.entity.MenuReview;
import club._8b1t.service.MenuReviewService;
import club._8b1t.mapper.MenuReviewMapper;
import org.springframework.stereotype.Service;

/**
* @author root
* @description 针对表【menu_review(菜品评价表)】的数据库操作Service实现
* @createDate 2025-03-16 16:07:17
*/
@Service
public class MenuReviewServiceImpl extends ServiceImpl<MenuReviewMapper, MenuReview>
    implements MenuReviewService{

}




